 ...
 * @param {Object} config.sentry Raven
...
Logger.prototype.setConfig = function(config) {
  ...
  // Configure Raven client for Sentry
  // > require process.env.SENTRY_DSN to be set
  if (config.sentry && config.sentry.dsn) {
    this._raven = new raven.Client(config.sentry.dsn,config.sentry);
  } else {
    delete this._raven;
  }
};
 ...
 */
Logger.prototype.fatal = function (error , kwargs , message) {
  var args = Array.prototype.slice.call(arguments);
  ...
  // Send the exception before the stack trace because
  // the raven client is doing it on his own:
  if (this._raven) {
    this._raven.captureException(error, {
      message: 'string' === typeof(args[2]) ? args[2] : args[1],
      extra: 'object' === typeof(args[0]) ? args[0] : {}
    });
  }
 ...
 * @param  {String} message Error message
 */
Logger.prototype.error = function (error , kwargs , message) {
  var args = Array.prototype.slice.call(arguments);
 ...
 * @param  {String} message Error message
 */
Logger.prototype.warn = function (kwargs, message) {
  var args = Array.prototype.slice.call(arguments);

  if (this._raven && 40 >= this._config.sentry.level) {
    this._raven.captureMessage(args[1],{
      message: message,
      extra: 'object' === typeof(args[0]) ? args[0] : {}
    });
  }
 ...
 * @param  {String} message Error message
 */
Logger.prototype.info = function (kwargs, message) {
  var args = Array.prototype.slice.call(arguments);
  args.unshift('info');
  return this.log.apply(this, args);
};
 ...
 * @param  {String} message Error message
 */
Logger.prototype.debug = function (kwargs, message) {
  var args = Array.prototype.slice.call(arguments);
  args.unshift('debug');
  return this.log.apply(this, args);
};
 ...
 * @param  {String} message Error message
 */
Logger.prototype.trace = function (kwargs, message) {
  var args = Array.prototype.slice.call(arguments);
...
Logger.prototype.replaceGlobal = function(_global) {
  ...
  _global.console.log = function() {
    ...
    if ( -1 === ['metric','trace','debug','info','warn','error','fatal'].indexOf(args[0])){
      args.unshift('info');
    }

    var level = args.shift();
    if ( 'string' === typeof(args[0]) ) {
      if ( -1 !== args[0].indexOf('ALERT') ) {
        level = 'fatal';
      } else if ( -1 !== args[0].indexOf('ERROR') ) {
        level = 'error';
      } else if ( -1 !== args[0].indexOf('Error') ) {
        level = 'error';
      } else if ( -1 !== args[0].indexOf('exception') ) {
        level = 'error';
      } else if ( -1 !== args[0].toUpperCase().indexOf('WARN') ) {
        level = 'warn';
      }
    }
